...
# Run following command if needed: install.packages("writexl")
...
# Lag HPI using mutate + lag() functions 

housing.data <- housing.data %>% 
  group_by(state) %>% 
  mutate(lagged.index_nsa = lag(index_nsa))

# Quarter-over-quarter pct. change in HPI

housing.data <- housing.data %>% 
  group_by(state) %>% 
  mutate(pct.change.index_nsa = 
           100*(index_nsa - lagged.index_nsa)/lagged.index_nsa)
...
# a new variable named index

summary.stats <- group_by(housing.data, yr) %>% 
  summarize(avg.annual.index = mean(index_nsa))
