...
__HINT:__ You can use `&&` to add multiple restrictions in your `filter()` command.
...
__HINT:__ You could use the `&&`-syntax from (3) above, but that would get cumbersome if you had a longer list of states. Instead, take a look how we defined `b.vector` above. 
...
__HINT:__ Take a look at the `Group Data` section of the `dplyr` cheat sheet. We want to group the data by year, then calculate average wages using the `summarise()` command.
